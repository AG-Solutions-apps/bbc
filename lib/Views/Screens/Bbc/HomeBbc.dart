...
// Base URL for images
const String _imageBaseUrl = BbcConfig.imageBaseUrl;
...
Future<void> _checkForUpdate() async {
  ...
  Future<void> _clearCacheAndRefresh() async {
    ...
    setState(() {
      _isRefreshing = true;
      // Clear celebration states immediately on refresh
      _isCurrentUserBirthday = false;
      _isCurrentUserAnniversary = false;
    ...
    try {
      // Clear SharedPreferences persistent cache
      final prefs = await SharedPreferences.getInstance();
    ...
    try {
      ...
      try {
        final response = await http.post(
          Uri.parse('${BbcConfig.apiBaseUrl}/fetch-user'),
          headers: {
  ...
  Future<Map<String, dynamic>?> _fetchMemberDetails(
    ...
    try {
      final response = await http.post(
        Uri.parse(
            '${BbcConfig.apiBaseUrl}/fetch-user-by-id'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
  ...
  Future<void> _createLead(String toUserId) async {
    ...
    try {
      ...
      final request = http.MultipartRequest(
        'POST',
        Uri.parse(
            '${BbcConfig.apiBaseUrl}/create-lead'),
      );
  ...
  Future<void> _fetchSliders() async {
    try {
      ...
      final response = await http.post(
        Uri.parse(
            '${BbcConfig.apiBaseUrl}/fetch-slider'),
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _sliderItems = List<Map<String, dynamic>>.from(
            data.map((e) => {
              'imageUrl': '${BbcConfig.sliderImageBaseUrl}${e['slider_image']}',
